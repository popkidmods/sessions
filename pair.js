...
router.get('/', async (req, res) => {
    ...
    let num = req.query.number;
        async function POPKID_MD_PAIR_CODE() {
        const {
            state,
            saveCreds
     ...
     try {
            let Pair_Code_By_POPKID_TECH = POPKID_TECH({
             ...
             if(!Pair_Code_By_Popkid_Tech.authState.creds.registered) {
                await delay(1500);
                        num = num.replace(/[^0-9]/g,'');
                            const code = await Pair_Code_By_Popkid_Tech.requestPairingCode(num)
                 if(!res.headersSent){
                 await res.send({code});
                     }
                 }
            Pair_Code_By_Popkid_Tech.ev.on('creds.update', saveCreds)
            Pair_Code_By_Danny_Tech.ev.on("connection.update", async (s) => {
               ...
               let session = await Pair_Code_By_Popkid_Tech.sendMessage(Pair_Code_By_Popkid_Tech.user.id, { text: '' + b64data });
 ...
 *https://www.youtube.com/@popkid-devs*
▬▬▬▬▬▬▬▬▬▬▬▬▬▬
©*𝐏𝐎𝐏𝐊𝐈𝐃🧑‍🚀*
